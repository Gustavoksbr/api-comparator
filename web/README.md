...
O projeto consome a API cujo código está em [`../api`](../api) (agora em PHP). A URL da API não fica mais fixa no código — veja a seção [🔧 Configuração da URL da API](#-configuração-da-url-da-api).
...
Testei endpoints chamando o back end real. Como cada visitante tem seu próprio sandbox
de cursos, isolado dos demais (veja `api/README.md`), criar/alterar/deletar não afeta
ninguém além de quem está rodando o teste — inclusive os cursos de seed (`c1` a `c10`),
que não são mais somente leitura.
