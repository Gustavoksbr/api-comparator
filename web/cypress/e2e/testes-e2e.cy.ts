...
describe('Procurar Cursos - Fluxo Completo', () => {
  ...
  it('Deve buscar cursos por substring de título', () => {
    ...
    // Campo de seleção (não o argumento "descricao: ...") tem indentação e
    // formato próprios (sem ":"), o que permite distinguir os dois na asserção.
    const linhaDoCampo = (nome: string) => `            ${nome}`;

    cy.get('[data-test-id="body-request-textarea-graphql"]')
      .should('contain', 'titulo: "Node"')
      .and('contain', linhaDoCampo('codigo'))
      .and('contain', linhaDoCampo('titulo'))
      .and('contain', linhaDoCampo('descricao'))
      .and('contain', linhaDoCampo('cargaHoraria'));
    ...
    cy.get('[data-test-id="body-request-textarea-graphql"]')
      ...
      .and('contain', linhaDoCampo('titulo'))
      .and('contain', linhaDoCampo('cargaHoraria'))
      .and('not.contain', linhaDoCampo('descricao'));
...
describe('Fluxo Completo com Erros (criar curso codigo duplicado) e sandbox isolado (alterar/deletar curso de seed)', () => {
  ...
  it('Deve conseguir alterar um curso de seed (c1) — sandbox isolado, sem restrição de somente leitura — e nao deve perder as informacoes ao mudar de caso de uso', () => {
    const codigo = 'c1';
    ...
    cy.get('[data-test-id="body-response-textarea-rest"]').should('contain', 'Novo Título'); // Verificar se o body response da alteração ainda esta presente

    cy.get('[data-test-id="botao-procurar"]').click();
    cy.get('[data-test-id="url-da-api-rest"]').should('contain', 'c2')
      .and('not.contain', 'Novo Titulo');
  });
  it('Deve conseguir deletar um curso de seed (c8) em REST, e depois confirmar em GraphQL/SOAP que ele não existe mais no sandbox', () => {
    cy.visit(BASE_URL);
    cy.get('[data-test-id="botao-deletar"]').click();

    cy.get('[data-test-id="deletar-input-codigo"]').type('c8');

    // Deletar via REST - Status 204
    cy.get('[data-test-id="api-type-header-rest"]').click();
    cy.wait(200);
    ...
    // Confirmar via GraphQL que já não existe mais (mesmo sandbox)
    cy.get('[data-test-id="botao-procurar"]').click();
    cy.get('[data-test-id="procurar-input-codigo"]').type('c8');
    cy.get('[data-test-id="api-type-header-graphql"]').click();
    cy.wait(200);
    ...
    cy.get('[data-test-id="body-response-textarea-graphql"]')
      .should('contain', 'errors')
      .and('contain', 'Curso não encontrado');

    // Confirmar via SOAP também
    cy.get('[data-test-id="api-type-header-soap"]').click();
    cy.wait(200);
